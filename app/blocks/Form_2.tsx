...
function FormSecond({bgDark = false, hideLines =false}: { bgDark?: boolean, hideLines?: boolean }) {
  const {t} = useTranslation();
  const texts = t('form.text', {returnObjects: true});

  return (
    <div className={'container text-white relative z-[50] pt-[50px]'}>
      <div
        className={clsx('bg-dark lg:overflow-visible overflow-hidden lg:block flex flex-col-reverse  rounded-[25px]  backdrop-blur-[3px]  relative py-[44px] lg:px-[60px] px-[30px]', {
          'bg-opacity-[.3]': !bgDark
        })}>
        <div
        ...
        <div
          className={"lg:absolute relative lg:mb-0 mb-[-45px] max-w-fit pointer-events-none z-[55] bottom-0 right-0"}>
          <img src="/formApp.png"
               className={"relative z-10 max-w-fit sm:w-fit w-[280px]"} alt=""/>
          <img src="/2_1.gif"
               className={"absolute sm:top-[15px] top-[13px] sm:left-[19px] left-[16px] sm:w-fit w-[180px]"} alt=""/>
        </div>
        {!hideLines && (
