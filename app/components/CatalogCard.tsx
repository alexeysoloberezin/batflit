...
const CatalogCard = React.memo(({price, name, image, id, onAddRemove, value, isBlack}: Props) => {
  const handleRemove = () => {
    onAddRemove(value - 1)
  }
  ...
  return (
    <div className={clsx(" flex flex-col rounded-[20px] md:p-[25px] p-[15px]", {
      "bg-[#F1F1F1]": !isBlack,
      "bg-gray-2-gradient": isBlack
    })}>
      ...
      <div className={clsx(" text-[30px] font-bold", {
        "text-[#22AD7D]": !isBlack,
        "text-primary": isBlack
      })}>{price}₽</div>
      <div className={clsx(" text-[20px] mt-[8px] grow", {
        "text-black": !isBlack,
        "text-white": isBlack
      })}>{name}</div>
      <div className={clsx("flex items-center justify-center gap-[15px] mt-[8px] h-[37px] rounded-[7px]", {
        "bg-[#22AD7D]": !isBlack,
        "bg-primary": isBlack
      })}>
